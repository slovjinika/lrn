...
def choose_word(words, used_words, target_language):
    """Chooses a random word that has not been used yet, considering target language."""
    available_words = [word for word in words if word['en'] not in used_words and target_language in word]
    if not available_words:
        print("All words have already been used or are missing for the specified language!")
        return None
    return random.choice(available_words)

def generate_options(correct_word, words, display_language, number_of_options=8):
    """Generates a list of answer options, including the correct one."""
    key_to_use = 'en' if display_language == 'en' else display_language #dynamically getting language code
    correct_option = correct_word[key_to_use]
    options = [correct_option]

    # Remove correct word to not pick it as incorrect option
    available_words = [word for word in words if word[key_to_use] != correct_option and key_to_use in word]
...
def is_answer_correct(user_input, correct_answer_en, correct_answer_translation, display_language, options):
    """Checks if the user's answer is correct."""
    if user_input in [str(i+1) for i in range(len(options))]:
        try:
            chosen_option = options[int(user_input)-1].lower()
            correct_answer = correct_answer_en if display_language == 'en' else correct_answer_translation
...
def main():
    ...
    display_language = 'en' # Default to 'en' if no argument is specified
    if args.lang_ua:
      display_language = 'ua'
    elif args.lang_ru:
      display_language = 'ru'
    ...
    target_language = args.target_lang #what language we are learning
    ...
    while True:
        word = choose_word(words, used_words, target_language) #added target lang argument
        if not word:
            print("You win. All words have been learned.")
            break  # Exit the loop if all words have been used

        question_translation = word[target_language] if display_language == 'en' else word['en']  # What language to ask in

        options = generate_options(word, words, display_language) #generate options now uses display language
        ...
        while True:
            ...
            correct_answer_translation = word[target_language].lower()  # Correct answer (translation language) in lowercase
            ...
            if user_input == 's':
                speak_word(word['en'])
            elif is_answer_correct(user_input, correct_answer_en, correct_answer_translation, display_language, options):
